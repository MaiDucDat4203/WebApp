...
namespace SV21T1020880.BusinessLayers
{
    public static class OrderDataService
    {
        ...
        public static int InitOrderCustomer(int customerID, string deliveryProvince, string deliveryAddress, IEnumerable<OrderDetail> details)
        ...
        public static bool ShipOrder(int orderID, int shipperID)
        {
            ...
            Order? data = orderDB.Get(orderID);
            if (data == null ||
                (data.Status != Constants.ORDER_ACCEPTED && data.Status != Constants.ORDER_SHIPPING))
            {
                return false;
            }
        ...
        public static bool DeleteOrder(int orderID)
        {
            ...
            if (data == null || (data.Status != Constants.ORDER_INIT &&
                                 data.Status != Constants.ORDER_CANCEL &&
                                 data.Status != Constants.ORDER_REJECTED))
            {
                return false;
        ...
        {
            return orderDB.ListByCustomer(customerID).ToList(); // Chưa có hàm ListByCustomer
        }
